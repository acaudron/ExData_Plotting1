...
plot(strptime(paste(mydata$Date, mydata$Time), "%d/%m/%Y %H:%M:%S"), as.double(mydata$Voltage), type = "l", xlab = "datetime", ylab = "Voltage")
plot(strptime(paste(mydata$Date, mydata$Time), "%d/%m/%Y %H:%M:%S"), as.double(mydata$Global_reactive_power), type = "l", xlab = "datetime", ylab = "Global_reactive_power")
dev.off()
